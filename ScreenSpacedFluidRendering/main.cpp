...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	switch (key)
	{
		// Camera movement
	case GLFW_KEY_W:
		camVelocity.z += (action == GLFW_PRESS ? -1 : 1);
		break;
	case GLFW_KEY_A:
		camVelocity.x += (action == GLFW_PRESS ? -1 : 1);
		break;
	case GLFW_KEY_S:
		camVelocity.z += (action == GLFW_PRESS ? 1 : -1);
		break;
...
{
	camYaw += camRotationSpeed * (xpos - (double) previousMousePos.x);
	camPitch += camRotationSpeed * (ypos - (double) previousMousePos.y);
...
void initialize(GLFWwindow* window)
{
	...
	//Model::Load("../assimp--3.0.1270-sdk/test/models/X/Testwuson.X");
	//Model::Load("../assimp--3.0.1270-sdk/test/models-nonbsd/X/dwarf.x");
	dwarf = Model::Load("../assimp--3.0.1270-sdk/test/models-nonbsd/X/dwarf.x");
...
void update(float elapsedTime){
	...
	camPos += camSpeed * elapsedTime * camVelocity *
